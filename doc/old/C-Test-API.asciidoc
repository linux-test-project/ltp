...
module. For this cases a mapping of kernel config options to modules is
maintained in the kconfig checker. When a kernel config option is requested by
a test that has associated mapping in the kconfig source the ``modules.dep``
file is checked for module presence when the option is set to ``m``. The check
is skipped on Android.
