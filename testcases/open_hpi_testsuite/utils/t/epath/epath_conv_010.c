...
{
        char *test_string = "{SYSTEM_CHASSIS,4}{DISK_DRIVE,48}";
	...
	if (err) {
        ...
        if (ep.Entry[0].EntityType != SAHPI_ENT_DISK_DRIVE) {
	    printf("  Error! Testcase failed. Line=%d\n", __LINE__);
	    printf("  Received=%d; Expected=%d\n", ep.Entry[0].EntityType, SAHPI_ENT_DISK_DRIVE);
	    return -1;
	}
                
        if (ep.Entry[0].EntityLocation != 48) {
	    printf("  Error! Testcase failed. Line=%d\n", __LINE__);
	    printf("  Received=%d; Expected=%d\n", ep.Entry[0].EntityLocation, 48);
	    return -1;
	}
