...
{
        char *test_string = "{OTHER_SYSTEM_BOARD,90}{MEMORY_MODULE,12}";
	...
	if (err) {
        ...
        if (ep.Entry[0].EntityLocation != 12) {
	    printf("  Error! Testcase failed. Line=%d\n", __LINE__);
	    printf("  Received=%d; Expected=%d\n", ep.Entry[0].EntityLocation, 12);
	    return -1;
	}
        
        if (ep.Entry[1].EntityType != SAHPI_ENT_OTHER_SYSTEM_BOARD) {
	    printf("  Error! Testcase failed. Line=%d\n", __LINE__);
	    printf("  Received=%d; Expected=%d\n", ep.Entry[1].EntityType, SAHPI_ENT_OTHER_SYSTEM_BOARD);
	    return -1;
	}
