...
{
        char *test_string = "{SWITCH,39}{PROCESSOR_BOARD,20}";
	...
	if (err) {
        ...
        if (ep.Entry[0].EntityType != SAHPI_ENT_PROCESSOR_BOARD) {
	    printf("  Error! Testcase failed. Line=%d\n", __LINE__);
	    printf("  Received=%d; Expected=%d\n", ep.Entry[0].EntityType, SAHPI_ENT_PROCESSOR_BOARD);
	    return -1;
	}
                
        if (ep.Entry[0].EntityLocation != 20) {
	    printf("  Error! Testcase failed. Line=%d\n", __LINE__);
	    printf("  Received=%d; Expected=%d\n", ep.Entry[0].EntityLocation, 20);
	    return -1;
	}
        ...
        if (ep.Entry[1].EntityLocation != 39) {
	    printf("  Error! Testcase failed. Line=%d\n", __LINE__);
	    printf("  Received=%d; Expected=%d\n", ep.Entry[1].EntityLocation, 39);
	    return -1;
	}
