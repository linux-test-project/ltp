...
{
        char *test_string = "{RACK,66}{IO_BLADE,57}";
	...
	if (err) {
        ...
        if (ep.Entry[0].EntityType != SAHPI_ENT_IO_BLADE) {
	    printf("  Error! Testcase failed. Line=%d\n", __LINE__);
        ...
        if (ep.Entry[1].EntityType != SAHPI_ENT_RACK) {
	    printf("  Error! Testcase failed. Line=%d\n", __LINE__);
	    printf("  Received=%d; Expected=%d\n", ep.Entry[1].EntityType, SAHPI_ENT_RACK);
	    return -1;
	}
