...
{
        char *test_string = "{ADD_IN_CARD,43}{CHASSIS_BACK_PANEL_BOARD,83}";
	...
	if (err) {
        ...
        if (ep.Entry[0].EntityLocation != 83) {
	    printf("  Error! Testcase failed. Line=%d\n", __LINE__);
	    printf("  Received=%d; Expected=%d\n", ep.Entry[0].EntityLocation, 83);
	    return -1;
	}
        ...
        if (ep.Entry[1].EntityLocation != 43) {
	    printf("  Error! Testcase failed. Line=%d\n", __LINE__);
	    printf("  Received=%d; Expected=%d\n", ep.Entry[1].EntityLocation, 43);
	    return -1;
	}
