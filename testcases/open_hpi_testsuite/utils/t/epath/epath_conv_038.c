...
{
        char *test_string = "{SUB_CHASSIS,18}{SYS_EXPANSION_BOARD,54}";
	...
	if (err) {
        ...
        if (ep.Entry[0].EntityType != SAHPI_ENT_SYS_EXPANSION_BOARD) {
	    printf("  Error! Testcase failed. Line=%d\n", __LINE__);
	    printf("  Received=%d; Expected=%d\n", ep.Entry[0].EntityType, SAHPI_ENT_SYS_EXPANSION_BOARD);
	    return -1;
	}
                
        if (ep.Entry[0].EntityLocation != 54) {
	    printf("  Error! Testcase failed. Line=%d\n", __LINE__);
	    printf("  Received=%d; Expected=%d\n", ep.Entry[0].EntityLocation, 54);
	    return -1;
	}
        
        if (ep.Entry[1].EntityType != SAHPI_ENT_SUB_CHASSIS) {
	    printf("  Error! Testcase failed. Line=%d\n", __LINE__);
