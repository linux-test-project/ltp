...
{
        char *test_string = "{COOLING_UNIT,79}{CHASSIS_SPECIFIC,79}";
	...
	if (err) {
        ...
        if (ep.Entry[0].EntityLocation != 79) {
	    printf("  Error! Testcase failed. Line=%d\n", __LINE__);
	    printf("  Received=%d; Expected=%d\n", ep.Entry[0].EntityLocation, 79);
	    return -1;
	}
        ...
        if (ep.Entry[1].EntityLocation != 79) {
	    printf("  Error! Testcase failed. Line=%d\n", __LINE__);
	    printf("  Received=%d; Expected=%d\n", ep.Entry[1].EntityLocation, 79);
	    return -1;
	}
