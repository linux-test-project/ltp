...
{
        char *test_string = "{SPEC_PROC_BLADE,68}{DISK_BAY,82}";
	...
	if (err) {
        ...
        if (ep.Entry[0].EntityLocation != 82) {
	    printf("  Error! Testcase failed. Line=%d\n", __LINE__);
	    printf("  Received=%d; Expected=%d\n", ep.Entry[0].EntityLocation, 82);
	    return -1;
	}
        ...
        if (ep.Entry[1].EntityLocation != 68) {
	    printf("  Error! Testcase failed. Line=%d\n", __LINE__);
	    printf("  Received=%d; Expected=%d\n", ep.Entry[1].EntityLocation, 68);
	    return -1;
	}
