...
{
        char *test_string = "{DEVICE_BAY,69}{OTHER_CHASSIS_BOARD,50}";
	...
	if (err) {
        ...
        if (ep.Entry[1].EntityType != SAHPI_ENT_DEVICE_BAY) {
	    printf("  Error! Testcase failed. Line=%d\n", __LINE__);
	    printf("  Received=%d; Expected=%d\n", ep.Entry[1].EntityType, SAHPI_ENT_DEVICE_BAY);
	    return -1;
	}
        
        if (ep.Entry[1].EntityLocation != 69) {
	    printf("  Error! Testcase failed. Line=%d\n", __LINE__);
	    printf("  Received=%d; Expected=%d\n", ep.Entry[1].EntityLocation, 69);
	    return -1;
	}
