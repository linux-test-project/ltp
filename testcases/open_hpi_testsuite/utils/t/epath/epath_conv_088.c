...
{
        char *test_string = "{POWER_DISTRIBUTION_UNIT,45}{CHASSIS_SPECIFIC,26}";
	...
	if (err) {
        ...
        if (ep.Entry[0].EntityLocation != 26) {
	    printf("  Error! Testcase failed. Line=%d\n", __LINE__);
	    printf("  Received=%d; Expected=%d\n", ep.Entry[0].EntityLocation, 26);
	    return -1;
	}
        ...
        if (ep.Entry[1].EntityLocation != 45) {
	    printf("  Error! Testcase failed. Line=%d\n", __LINE__);
