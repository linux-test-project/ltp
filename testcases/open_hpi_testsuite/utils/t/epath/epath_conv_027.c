...
{
        char *test_string = "{OTHER,55}{SYSTEM_BOARD,63}";
	...
	if (err) {
        ...
        if (ep.Entry[0].EntityLocation != 63) {
	    printf("  Error! Testcase failed. Line=%d\n", __LINE__);
	    printf("  Received=%d; Expected=%d\n", ep.Entry[0].EntityLocation, 63);
	    return -1;
	}
        ...
        if (ep.Entry[1].EntityLocation != 55) {
	    printf("  Error! Testcase failed. Line=%d\n", __LINE__);
	    printf("  Received=%d; Expected=%d\n", ep.Entry[1].EntityLocation, 55);
	    return -1;
	}
