...
{
        char *test_string = "{INTERCONNECT,94}{DEVICE_BAY,18}";
	...
	if (err) {
        ...
        if (ep.Entry[0].EntityType != SAHPI_ENT_DEVICE_BAY) {
	    printf("  Error! Testcase failed. Line=%d\n", __LINE__);
        ...
        if (ep.Entry[1].EntityLocation != 94) {
	    printf("  Error! Testcase failed. Line=%d\n", __LINE__);
	    printf("  Received=%d; Expected=%d\n", ep.Entry[1].EntityLocation, 94);
	    return -1;
	}
