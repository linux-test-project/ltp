...
{
        char *test_string = "{SYS_MGMNT_MODULE,71}{COMPACTPCI_CHASSIS,87}";
	...
	if (err) {
        ...
        if (ep.Entry[0].EntityType != SAHPI_ENT_COMPACTPCI_CHASSIS) {
	    printf("  Error! Testcase failed. Line=%d\n", __LINE__);
	    printf("  Received=%d; Expected=%d\n", ep.Entry[0].EntityType, SAHPI_ENT_COMPACTPCI_CHASSIS);
	    return -1;
	}
                
        if (ep.Entry[0].EntityLocation != 87) {
	    printf("  Error! Testcase failed. Line=%d\n", __LINE__);
        ...
        if (ep.Entry[1].EntityLocation != 71) {
	    printf("  Error! Testcase failed. Line=%d\n", __LINE__);
	    printf("  Received=%d; Expected=%d\n", ep.Entry[1].EntityLocation, 71);
	    return -1;
	}
