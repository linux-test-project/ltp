...
{
        char *test_string = "{SPEC_PROC_BLADE,26}{BACK_PANEL_BOARD,36}";
	...
	if (err) {
        ...
        if (ep.Entry[0].EntityLocation != 36) {
	    printf("  Error! Testcase failed. Line=%d\n", __LINE__);
	    printf("  Received=%d; Expected=%d\n", ep.Entry[0].EntityLocation, 36);
	    return -1;
	}
        
        if (ep.Entry[1].EntityType != SAHPI_ENT_SPEC_PROC_BLADE) {
	    printf("  Error! Testcase failed. Line=%d\n", __LINE__);
	    printf("  Received=%d; Expected=%d\n", ep.Entry[1].EntityType, SAHPI_ENT_SPEC_PROC_BLADE);
	    return -1;
	}
        
        if (ep.Entry[1].EntityLocation != 26) {
	    printf("  Error! Testcase failed. Line=%d\n", __LINE__);
