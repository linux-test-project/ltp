...
{
        char *test_string = "{COOLING_DEVICE,18}{PROCESSOR,42}";
	...
	if (err) {
        ...
        if (ep.Entry[0].EntityLocation != 42) {
	    printf("  Error! Testcase failed. Line=%d\n", __LINE__);
	    printf("  Received=%d; Expected=%d\n", ep.Entry[0].EntityLocation, 42);
	    return -1;
	}
        
        if (ep.Entry[1].EntityType != SAHPI_ENT_COOLING_DEVICE) {
	    printf("  Error! Testcase failed. Line=%d\n", __LINE__);
	    printf("  Received=%d; Expected=%d\n", ep.Entry[1].EntityType, SAHPI_ENT_COOLING_DEVICE);
	    return -1;
	}
        
        if (ep.Entry[1].EntityLocation != 18) {
	    printf("  Error! Testcase failed. Line=%d\n", __LINE__);
