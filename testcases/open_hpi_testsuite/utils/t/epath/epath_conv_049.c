...
{
        char *test_string = "{OTHER_CHASSIS_BOARD,50}{BACK_PANEL_BOARD,80}";
	...
	if (err) {
        ...
        if (ep.Entry[0].EntityType != SAHPI_ENT_BACK_PANEL_BOARD) {
	    printf("  Error! Testcase failed. Line=%d\n", __LINE__);
        ...
        if (ep.Entry[1].EntityType != SAHPI_ENT_OTHER_CHASSIS_BOARD) {
	    printf("  Error! Testcase failed. Line=%d\n", __LINE__);
	    printf("  Received=%d; Expected=%d\n", ep.Entry[1].EntityType, SAHPI_ENT_OTHER_CHASSIS_BOARD);
	    return -1;
	}
        
        if (ep.Entry[1].EntityLocation != 50) {
	    printf("  Error! Testcase failed. Line=%d\n", __LINE__);
	    printf("  Received=%d; Expected=%d\n", ep.Entry[1].EntityLocation, 50);
	    return -1;
	}
