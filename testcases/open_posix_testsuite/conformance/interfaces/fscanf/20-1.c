 ...
 *	control string format and a set of pointer arguments indicating where
...
int main(void)
{
	...
	if (ret == 3 && min_value == UCHAR_MIN_VALUE &&
		...
		printf(TNAME " Expected values: ret = 3, max_value = %hhX, "
			...
			"Obtained values: ret = %d, max_value = %hhX, "
			"min_value = %hhX, mid_value = %hhX\n", UCHAR_MAX,
			UCHAR_MIN_VALUE, UCHAR_CONST_VALUE, ret, max_value,
			min_value, mid_value);
		exit(PTS_FAIL);
	}
}
