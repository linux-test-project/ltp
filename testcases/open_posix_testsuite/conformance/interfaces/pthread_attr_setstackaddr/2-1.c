 ...
 * 4.  In the created thread, read the stackaddr
 *
...
#define TEST "2-1"
#define FUNCTION "pthread_attr_setstackaddr"
...
int main()
{
	...
	// Got segmentation fault when COEF < 27 in glibc.
	ssize = PTHREAD_STACK_MIN + COEF * sysconf(_SC_PAGE_SIZE);
	...
	rc = pthread_attr_setstackaddr(&attr, stack_addr);
        if (rc != 0 ) {
                perror(ERROR_PREFIX "pthread_attr_setstackaddr");
                exit(PTS_UNRESOLVED);
        }
