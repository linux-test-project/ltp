 ...
 * source tree.
  ...
  3. (child) Setup a signal handler for SIGBUS with SA_NODEFER set
     in the sa_flags field
  4. (child) raise SIGBUS
  5. (child, signal handler) increment handler count
  6. (child, signal handler) if count is 1 then raise SIGBUS
...
void handler(int signo LTP_ATTRIBUTE_UNUSED)
{
	...
	if (handler_count == 1) {
		...
		raise(SIGBUS);
		printf("Returning from raising SIGBUS\n");
	}
