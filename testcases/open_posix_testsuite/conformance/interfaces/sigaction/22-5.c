 ...
 * source tree.
  ...
  3. (child) Setup a signal handler for SIGCONT with SA_NODEFER set
     in the sa_flags field
  4. (child) raise SIGCONT
  5. (child, signal handler) increment handler count
  6. (child, signal handler) if count is 1 then raise SIGCONT
...
void handler(int signo LTP_ATTRIBUTE_UNUSED)
{
	...
	if (handler_count == 1) {
		printf("Raising SIGCONT\n");
		raise(SIGCONT);
		printf("Returning from raising SIGCONT\n");
	}
