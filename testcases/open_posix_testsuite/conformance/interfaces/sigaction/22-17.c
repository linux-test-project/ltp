 ...
 * source tree.
  ...
  3. (child) Setup a signal handler for SIGUSR1 with SA_NODEFER set
     in the sa_flags field
  4. (child) raise SIGUSR1
  5. (child, signal handler) increment handler count
  6. (child, signal handler) if count is 1 then raise SIGUSR1
...
void handler(int signo LTP_ATTRIBUTE_UNUSED)
{
	...
	if (handler_count == 1) {
		...
		raise(SIGUSR1);
		printf("Returning from raising SIGUSR1\n");
	}
