 ...
 * source tree.
  ...
  3. (child) Setup a signal handler for SIGSEGV with SA_NODEFER set
     in the sa_flags field
  4. (child) raise SIGSEGV
  5. (child, signal handler) increment handler count
  6. (child, signal handler) if count is 1 then raise SIGSEGV
...
void handler(int signo LTP_ATTRIBUTE_UNUSED)
{
	...
	if (handler_count == 1) {
		...
		raise(SIGSEGV);
		printf("Returning from raising SIGSEGV\n");
	}
