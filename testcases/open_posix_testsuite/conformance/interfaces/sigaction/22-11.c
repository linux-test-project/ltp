 ...
 * source tree.
  ...
  3. (child) Setup a signal handler for SIGQUIT with SA_NODEFER set
     in the sa_flags field
  4. (child) raise SIGQUIT
  5. (child, signal handler) increment handler count
  6. (child, signal handler) if count is 1 then raise SIGQUIT
...
void handler(int signo LTP_ATTRIBUTE_UNUSED)
{
	...
	if (handler_count == 1) {
		...
		raise(SIGQUIT);
		printf("Returning from raising SIGQUIT\n");
	}
