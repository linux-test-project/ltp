 ...
 * source tree.
  ...
  3. (child) Setup a signal handler for SIGTTOU with SA_NODEFER set
     in the sa_flags field
  4. (child) raise SIGTTOU
  5. (child, signal handler) increment handler count
  6. (child, signal handler) if count is 1 then raise SIGTTOU
...
void handler(int signo LTP_ATTRIBUTE_UNUSED)
{
	...
	if (handler_count == 1) {
		printf("Raising SIGTTOU\n");
		raise(SIGTTOU);
		printf("Returning from raising SIGTTOU\n");
	}
