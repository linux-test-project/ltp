 ...
 * source tree.
  ...
  3. (child) Setup a signal handler for SIGURG with SA_NODEFER set
     in the sa_flags field
  4. (child) raise SIGURG
  5. (child, signal handler) increment handler count
  6. (child, signal handler) if count is 1 then raise SIGURG
...
void handler(int signo LTP_ATTRIBUTE_UNUSED)
{
	...
	if (handler_count == 1) {
		printf("Raising SIGURG\n");
		raise(SIGURG);
		printf("Returning from raising SIGURG\n");
	}
