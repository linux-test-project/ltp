 ...
 *	to by s2. Upon completion, function will return an integer less than 0,
 *	if the string pointed to by s1 is less than the string pointed to by s2.
 *
 *  method:
 *	-Generate sample string s1 with all lower case characters.
 *	-Generate sample string s2 with all upper case characters.
...
int main(void)
{
	...
	for (i = 1; i < STRING_MAX_LEN; i += STEP_COUNT) {
		...
		sample_str_2 = random_string(i, false);
		sample_str_1 = random_string(i, true);
		ret = strcmp(sample_str_1, sample_str_2);

		if (ret >= 0) {
			printf(TNAME " Test Failed, return value is not as expected, "
						"for string1 less than string2. Expected < 0,"
						"but got %d.\n", ret);
			exit(PTS_FAIL);
		}
		free(sample_str_1);
