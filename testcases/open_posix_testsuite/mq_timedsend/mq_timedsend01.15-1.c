 ...
 * messages until a failure is received.  Test that the error is
...
#ifndef MQ_OPEN_MAX	// required by POSIX, but redefine if not set
#define MQ_OPEN_MAX 50
#endif
...
int main()
{
	...
	int maxreached=0, i;
        ...
        gqueue = mq_open(gqname, O_CREAT | O_RDWR, S_IRUSR | S_IWUSR, NULL);
        if (gqueue == (mqd_t)-1) {
