 ...
 *  recvfrom02-sctp-udp [-c n] [-i n] [-I x] [-P x] [-t]
...
char *TCID="recvfrom02-sctp-udp";	/* Test program identifier.    */
int testno;

char	buf[1024];
int	s, cfd;	/* socket descriptor */
struct sockaddr_in6 sin1, from, to;
int	fromlen;
...
struct test_case_t {		/* test case structure */
	...
	{ PF_INET6, SOCK_SEQPACKET, IPPROTO_SCTP, (void *)buf, sizeof(buf), 0,
	...
	{ PF_INET6, SOCK_SEQPACKET, IPPROTO_SCTP, (void *)buf, sizeof(buf), -1,
		(struct sockaddr *)&from, &fromlen,
		-1, EINVAL, setup1, cleanup1, "invalid flags set" },
	{ PF_INET6, SOCK_SEQPACKET, IPPROTO_SCTP, (void *)buf, sizeof(buf), 
...
pid_t start_client(void) 
{
	...
	char*	message = "THIS IS FOR RECVFROM";
	
	cfd = socket(PF_INET6, SOCK_SEQPACKET, IPPROTO_SCTP);
	if (cfd < 0) {
	...
	to.sin6_port = htons(PORT);
        if (!(inet_pton (AF_INET6, v6_local_host, &to.sin6_addr))) {
	...
	local.sin6_addr = (struct in6_addr) IN6ADDR_ANY_INIT;
	...
	while (1) {
		sleep(1);
		/* send something back */
		if( sendto(cfd, message, sizeof(message), 0,
