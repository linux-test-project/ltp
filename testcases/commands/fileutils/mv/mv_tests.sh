...
init()
{
	...
	mkdir -p $LTPTMP/tst_mv.tmp &> $LTPTMP/tst_mv.err || RC=$? 
	if [ $RC -ne 0 ]
	then
		$LTPBIN/tst_brk TBROK $LTPTMP/tst_mv.err NULL \
...
creat_dirnfiles()
{
	...
	do
        ...
        mkdir -p $dirname  &>$LTPTMP/tst_mv.err || RC=$?
		if [ $RC -ne 0 ]
		then
			$LTPBIN/tst_brk TBROK $LTPTMP/tst_mv.err NULL \
...
test01()
{
	...
	mv $LTPTMP/tst_mv.tmp $LTPTMP/tst_mv.tmp1 &>$LTPTMP/tst_mv.err || RC=$?
    if [ $RC -ne 0 ]
	then
	...
	ls -R $LTPTMP/tst_mv.tmp1 &>$LTPTMP/tst_mv.out
	...
	diff -w -B -q $LTPTMP/tst_mv.out $LTPTMP/tst_mv.exp &>$LTPTMP/tst_mv.err \
		|| RC=$?
	if [ $RC -ne 0 ]
	then
		$LTPBIN/tst_res TFAIL $LTPTMP/tst_mv.err \
