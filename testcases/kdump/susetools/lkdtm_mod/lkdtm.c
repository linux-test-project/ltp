 ...
 * Changes to make it work on SuSE :  Sachin Sant <sachinp@in.ibm.com>
 *
...
int lkdtm_module_init(void)
{
	...
	case INT_HARDWARE_ENTRY:
		...
		break;
	case INT_HW_IRQ_EN:
		addr = kallsyms_lookup_name("handle_IRQ_event");
		if (addr) {
			*(lkdtm.kp.addr) = addr;
			lkdtm.entry = JPROBE_ENTRY(jp_handle_irq_event);
		} else
			printk(KERN_INFO "lkdtm : Crash point not available\n");
		break;
	case INT_TASKLET_ENTRY:
		addr = kallsyms_lookup_name("tasklet_action");
		if (addr) {
			*(lkdtm.kp.addr) = addr;
			lkdtm.entry = JPROBE_ENTRY(jp_tasklet_action);
		} else
			printk(KERN_INFO "lkdtm : Crash point not available\n");
		break;
	case FS_DEVRW:
		...
		break;
	case MEM_SWAPOUT:
		addr = kallsyms_lookup_name("shrink_inactive_list");
		if (addr) {
			*(lkdtm.kp.addr) = addr;
			lkdtm.entry = JPROBE_ENTRY(jp_shrink_page_list);
		} else
			printk(KERN_INFO "lkdtm : Crash point not available\n");
		break;
	case TIMERADD:
		addr = kallsyms_lookup_name("hrtimer_start");
		if (addr) {
			*(lkdtm.kp.addr) = addr;
			lkdtm.entry = JPROBE_ENTRY(jp_hrtimer_start);
		} else
			printk(KERN_INFO "lkdtm : Crash point not available\n");
		break;
	case SCSI_DISPATCH_CMD:
		addr = kallsyms_lookup_name("scsi_dispatch_cmd");
		if (addr) {
			*(lkdtm.kp.addr) = addr;
			lkdtm.entry = JPROBE_ENTRY(jp_scsi_dispatch_cmd);
		} else
			printk(KERN_INFO "lkdtm : Crash point not available\n");
		break;
	case IDE_CORE_CP:
#ifdef CONFIG_IDE
		addr = kallsyms_lookup_name("generic_ide_ioctl");
		if (addr) {
			*(lkdtm.kp.addr) = addr;
			lkdtm.entry = JPROBE_ENTRY(jp_generic_ide_ioctl);
		} else
			printk(KERN_INFO "lkdtm : Crash point not available\n");
#else
		printk(KERN_INFO "lkdtm : Crash point not available\n");
