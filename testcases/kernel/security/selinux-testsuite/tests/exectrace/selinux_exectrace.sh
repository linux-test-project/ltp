...
test01()
{
	...
	if [ $RC -ne 0 ]
        then
                tst_resm TFAIL "Test #1: exectrace failed."
        else
                tst_resm TPASS "Test #1: exectrace passed."
        fi
        return $RC
}
...
test02()
{
	...
	if [ $RC -ne 0 ]
        then
                tst_resm TPASS "Test #2: exectrace passed."
		RC=0
        else
                tst_resm TFAIL "Test #2: exectrace failed."
		RC=1
        fi
	return $RC
}
