...
SETS_DEFAULTS="${TCID=write_freezing.sh} ${TST_COUNT=1} ${TST_TOTAL=1}"
declare -r TCID
declare -r TST_COUNT
declare -r TST_TOTAL
...
while /bin/true ; do
	...
	assert_cgroup_freezer_state "THAWED" "ERROR: writing FREEZING to freezer.state should not change freezer state (expected IO error)"
	result=$FINISHED
	break
done
trap '' ERR
