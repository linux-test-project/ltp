...
#include <sys/types.h>
#include "config.h"
#if defined(HAS_NEW_MINIMAL_QUOTACTL) || defined(HAS_RHEL48_QUOTACTL)
#include <sys/quota.h>
#elif defined(HAS_RHEL46_QUOTACTL)
#include <linux/quota.h>
#else
/* Not HAS_NEW_MINIMAL_QUOTACTL or HAS_RHEL48_QUOTACTL or HAS_RHEL46_QUOTACTL. */
#define BROKEN_QUOTACTL 1
#endif
#if defined(HAS_RHEL46_QUOTACTL) || defined(HAS_RHEL48_QUOTACTL)
#include <xfs/xqm.h>
#include <linux/dqblk_v1.h>
#include <linux/dqblk_v2.h>
#endif /* defined(HAS_RHEL46_QUOTACTL) || defined(HAS_RHEL48_QUOTACTL) */
	...
	for (lc = 0; TEST_LOOPING(lc); ++lc) {
		...
		for (testno = 0; testno < TST_TOTAL; ++testno) {
			...
			for (i = 0; i <= 7; i++){
				...
				TEST(retval = syscall(__NR_quotactl, cmd[i],
							(const char *)NULL, id,
							(caddr_t)NULL));
				if (TEST_RETURN != 0) {
