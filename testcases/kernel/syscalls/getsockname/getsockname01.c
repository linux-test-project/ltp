...
static struct test_case {
	...
	{ .sock = &sock_fake, .sockaddr = &fsin1, .addrlen = &sinlen, .experrno = EBADF, "bad file descriptor"},
	{ .sock = &sock_null, .sockaddr = &fsin1, .addrlen = &sinlen, .experrno = ENOTSOCK, "bad file descriptor"},
	{ .sock = &sock_bind, .sockaddr = NULL, .addrlen = &sinlen, .experrno = EFAULT, "invalid socket buffer"},
	{ .sock = &sock_bind, .sockaddr = &fsin1, .addrlen = NULL, .experrno = EFAULT, "invalid aligned salen"},
	{ .sock = &sock_bind, .sockaddr = &fsin1, .addrlen = (socklen_t *) 1, .experrno = EFAULT, "invalid unaligned salen"},
};
...
static void check_getsockname(unsigned int nr)
{
	...
	TST_EXP_FAIL(getsockname(*(tc->sock), (struct sockaddr *) tc->sockaddr, tc->addrlen), tc->experrno, "%s", tc->desc);
}
