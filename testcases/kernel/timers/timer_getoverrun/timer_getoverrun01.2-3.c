 ...
 * - Wait for that timer to expire expectedoverruns+1 times (wait valuensec + 
 *   (expectedoverruns)*intervalnsec).
...
int main()
{
	...
	printf("value = %d, interval = %d, expected overruns = %d\n",
			valuensec, intervalnsec, expectedoverruns);
