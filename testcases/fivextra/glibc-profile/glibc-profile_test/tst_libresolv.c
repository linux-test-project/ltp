...
int  chang_address_from_asc_to_binary( const char * input, struct in_addr * addr)
 {
     return inet_aton (input, addr);
  }
  ...
  for (cnt = 0; cnt < sizeof (tests) / sizeof (tests[0]); ++cnt)
    {
      ...
      if (chang_address_from_asc_to_binary (tests[cnt].input, &addr) != tests[cnt].valid)
	{
	  if (tests[cnt].valid)
