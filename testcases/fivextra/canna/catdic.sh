...
function test01()
{
	...
	# Register test case
	#
	tc_register "\"catdic\" Display all words in a dictionary"
	...
	tc_fail_if_bad $? "[catdic] Could not create a new dictionary" || return

	echo -e $WORD1 | addwords _testdic01 > /dev/null 2>&1
	tc_fail_if_bad $? "[catdic] Could not add a new word #1" || return

	echo -e $WORD2 | addwords _testdic01 > /dev/null 2>&1
	tc_fail_if_bad $? "[catdic] Could not add a new word #2" || return

	echo -e $WORD1 > $TCTMP/file1
	echo -e $WORD2 >> $TCTMP/file1
	catdic _testdic01 > $TCTMP/file2
	diff $TCTMP/file1 $TCTMP/file2 > /dev/null 2>&1 
	tc_pass_or_fail $? "[catdic] No words or unexpected words are registered."
}
...
function test02()
{
	...
	# Register test case
	#
	tc_register "\"catdic -G\" Display all words in a group dictionary"
	...
	tc_fail_if_bad $? "[catdic -G] Could not create a new dictionary" || return

	echo -e $WORD1 | addwords _testgdic01 > /dev/null 2>&1
	tc_fail_if_bad $? "[catdic -G] Could not add a new word #1" || return

	echo -e $WORD2 | addwords _testgdic01 > /dev/null 2>&1
	tc_fail_if_bad $? "[catdic -G] Could not add a new word #2" || return

	echo -e $WORD1 > $TCTMP/file1
	echo -e $WORD2 >> $TCTMP/file1
	catdic -G _testgdic01 > $TCTMP/file2
	diff $TCTMP/file1 $TCTMP/file2 > /dev/null 2>&1 
	tc_pass_or_fail $? "[catdic -G] No words or unexpected words are registered."
}
