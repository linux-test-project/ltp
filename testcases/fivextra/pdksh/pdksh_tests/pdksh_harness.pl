...
if (!&Getopts('C:p:Ps:t:ve:')) {
    system("tst_resm TBROK 'Incorrect invocation of test harness'");
    print STDERR $Usage;
    exit 1;
}

if (!defined $opt_p) {
    system("tst_resm TBROK '$prog: no program specified (use -p)'");
    die "$prog: no program specified (use -p)\n";
}
if (!defined $opt_s) {
    system("tst_resm TBROK '$prog: no test set specified (use -s)'");
...
if (defined $opt_t) {
    if ($opt_t !~ /^\d+$/ || $opt_t <= 0) {
	system("tst_resm TBROK '$prog: bad -t argument (should be number > 0): $opt_t'");
...
if (defined $opt_C) {
	...
	$c =~ s/\s+//;
	if ($c =~ /^!/) {
	    system("tst_resm TBROK '$prog: categories can't be negated on the command line'");
...
if (defined $opt_e) {
	...
	system("tst_resm TBROK '$0: bad -e argument: $opt_e'");
...
# Reset in run_test
###################
$ENV{'TCID'}="pdksh_test_harness";
$ENV{'TST_TOTAL'}="167";
$ENV{'TST_COUNT'}="2";

if (!mkdir($tempdir, 0777)) {
    system("tst_resm TBROK '$prog: couldn't make directory $tempdir - $!'");
    die "$prog: couldn't make directory $tempdir - $!\n";
}

chop($pwd = `pwd 2> /dev/null`);
if ($pwd eq '') {
    system("tst_resm TBROK '$prog: couldn't get current working directory'");
    die "$prog: couldn't get current working directory\n";
}
if (!chdir($pwd)) {
    system("tst_resm TBROK '$prog: couldn't cd to $pwd - $!'");
...
if (!$program_kludge) {
	...
	system("tst_resm TBROK '$prog: $test_prog is not executable'");
...
# add one for 1st test run from pdksh.sh
$tot_passed = $npassed + $nxpassed + 1;
...
process_test_dir
{
    ...
    if (!opendir(DIR, $dir)) {
	system("tst_resm TBROK '$prog: can't open directory $dir - $!'");
...
process_test_file
{
    ...
    if (!open(IN, $file)) {
	system("tst_resm TBROK '$prog: can't open $file - $!'");
...
run_test
{
    ...
    if (!chdir($tempdir)) {
	system("tst_resm TBROK '$prog: couldn't cd to $tempdir - $!'");
	...
	for ($i = 0; $i < $test{'file-setup'}; $i++) {
		...
		if (!chmod($perm, $name)) {
		    system("tst_resm TBROK '$prog:$test{':long-name'}: can't chmod $name - $!'");
		    print STDERR
		...
		if (!mkdir($name, $perm)) {
		    system("tst_resm TBROK '$prog:$test{':long-name'}: can't mkdir $name - $!'");
		    print STDERR
		...
		umask($oumask);
		if (!$ret) {
		    system("tst_resm TBROK '$prog:$test{':long-name'}: couldn't create symlink $name - $!'");
		    print STDERR
	...
	if ($@ ne '') {
	    system("tst_resm TBROK '$prog:$test{':long-name'}: error running perl-setup - $@'");
    ...
    $pid = fork;
    if (!defined $pid) {
	system("tst_resm TBROK '$prog: can't fork - $!'");
	...
	if (!open(STDIN, "< $ifile")) {
	    system("tst_resm TBROK '$prog: couldn't open $ifile in child - $!'");
	    print STDERR "$prog: couldn't open $ifile in child - $!\n";
	    kill('TERM', $$);
	}
	if (!open(STDOUT, "> $tempo")) {
	    system ("tst_resm TBROK '$prog: couldn't open $tempo in child - $!'");
	    print STDERR "$prog: couldn't open $tempo in child - $!\n";
	    kill('TERM', $$);
	}
	if (!open(STDERR, "> $tempe")) {
	    system("tst_resm TBROK '$prog: couldn't open $tempe in child - $!'");
	...
	system("tst_resm TBROK '$prog: couldn't execute $test_prog - $!'");
	kill('TERM', $$);
	exit(95);
    }
    $child_pid = $pid;
	...
	if ($xpid < 0) {
	    next if $! == &EINTR;
	    system("tst_resm TBROK '$prog: error waiting for child - $!'");
	...
	if ($@ ne '') {
	    system("tst_resm TBROK '$prog:$test{':long-name'}: error running perl-cleanup - $@'");
    ...
    # For LTP Harness Accounting.
    $num_tests++;
    $ENV{'TST_COUNT'}=$num_tests;

    if ($failed) {
	if (!$test{'expected-fail'}) {
	    #print "FAIL $name\n";
	    $nxfailed++;
	    system("tst_resm TFAIL '$name failed'");
      	} else {
	    #print "fail $name (as expected)\n";
	    system("tst_resm TPASS '$name failed as expected'");
	    $nfailed++;
	}
	$why = "\tDescription"
	...
	$nxpassed++;
	system("tst_resm TFAIL '$name passed unexpectedly'");
    } else {
	#print "pass $name\n";
	$npassed++;
	system("tst_resm TPASS '$name passed'");
    }
...
scrub_dir
{
    ...
    if (!opendir(DIR, $dir)) {
	system("tst_resm TBROK '$prog: couldn't open directory $dir - $!'");
	...
	if (-d $file) {
	    ...
	    if (!rmdir($file)) {
		system("tst_resm TBROK '$prog: couldn't rmdir $file - $!'");
		print STDERR "$prog: couldn't rmdir $file - $!\n";
		return undef;
	    }
	} else {
	    if (!unlink($file)) {
		system("tst_resm TBROK '$prog: couldn't unlink $file - $!'");
...
write_file
{
    ...
    print TEMP $str;
    if (!close(TEMP)) {
	system("tst_resm TBROK '$prog: error writing $file - $!'");
...
check_output
{
    ...
    if (!open(TEMP, "< $file")) {
	system("tst_resm TBROK '$prog:$name($what): couldn't open $file after running program - $!'");
...
compare_output
{
	...
	$ret = eval "$expect_pat";
	if ($@ ne '') {
	    system("tst_resm TBROK '$prog:$name($what): error evaluating $what pattern: $expect_pat - $@'");
...
eval_exit
{
    ...
    $w = eval $expr;
    if ($@ ne '') {
	system("tst_resm TBROK '$prog:$test{':long-name'}: bad expected-exit expression: $expect ($@)'");
...
read_test
{
	...
	if (!/^([-\w]+):\s*(|\S|\S.*\S)\s*$/) {
	    system("tst_resm TBROK '$prog:$file:$.: unrecognized line'");
	...
	if (!defined $flags) {
	    system("tst_resm TBROK '$prog:$file:$.: unrecognized field \"$field\"'");
	...
	if ($flags =~ /s/) {
	    ...
	    system("tst_resm TBROK '$prog:$file:$.: multiple \"$field\" fields'");
	...
	if ($val eq '' || $val eq '!' || $flags =~ /p/) {
		...
		if ($field eq 'file-setup') {
		    ...
		    #
		    if ($val !~ /^[ \t]*(\S+)[ \t]+(\S+)[ \t]+([^ \t].*)/) {
			system("tst_resm TBROK '$prog:$file:$.: bad paramter line for file-setup field'");
			print STDERR
			...
			system("tst_resm TBROK '$prog:$file:$.: bad file type for file-setup: $type'");
			print STDERR
			    "$prog:$file:$.: bad file type for file-setup: $type\n";
			return undef;
		    }
		    if ($perm !~ /^\d+$/) {
			system("tst_resm TBROK '$prog:$file:$.: bad permissions for file-setup: $type'");
			print STDERR
			...
			system("tst_resm TBROK '$prog:$file:$.: missing end quote for file name in file-setup: $rest'");
			print STDERR
			...
			system("tst_resm TBROK '$prog:$file:$.: file name in file-setup is absolute or contains ..: $name'");
			print STDERR
		...
		if ($field eq 'file-result') {
			...
			system("tst_resm TBROK '$prog:$file:$.: bad paramter line for file-result field'");
			print STDERR
			...
			= ($1, $2, $3, $4, $5, $6);
		    if ($type !~ /^(file|dir|symlink)$/) {
			system("tst_resm TBROK '$prog:$file:$.: bad file type for file-result: $type'");
			print STDERR
			    "$prog:$file:$.: bad file type for file-result: $type\n";
			return undef;
		    }
		    if ($perm !~ /^\d+$/ && $perm ne '*') {
			system("tst_resm TBROK '$file:$.: bad permissions for file-result: $perm'");
			print STDERR
			    "$prog:$file:$.: bad permissions for file-result: $perm\n";
			return undef;
		    }
		    if ($uid !~ /^\d+$/ && $uid ne '*') {
			system("tst_resm TBROK '$prog:$file:$.: bad user-id for file-result: $uid'");
			print STDERR
			    "$prog:$file:$.: bad user-id for file-result: $uid\n";
			return undef;
		    }
		    if ($gid !~ /^\d+$/ && $gid ne '*') {
			system("tst_resm TBROK '$prog:$file:$.: bad group-id for file-result: $gid'");
			print STDERR
			    "$prog:$file:$.: bad group-id for file-result: $gid\n";
			return undef;
		    }
		    if ($matchType !~ /^(exact|pattern)$/) {
			system("tst_resm TBROK '$prog:$file:$.: bad match type for file-result: $matchType'");
			print STDERR
		    ...
		    $c = substr($rest, 0, 1);
		    if (($len = index($rest, $c, 1) - 1) <= 0) {
			system("tst_resm TBROK '$prog:$file:$.: missing end quote for file name in file-result: $rest'");
			print STDERR
			...
			# check - a test can still use symlinks to get at files
			# outside the test directory.
			system("tst_resm TBROK '$prog:$file:$.: file name in file-result is absolute or contains ..: $name'");
			print STDERR
		...
		system("tst_resm TBROK '$prog:$file:$.: no value given for field \"$field\"'");
		print STDERR
    ...
    if ($_ eq '') {
	if (%test) {
	    system("tst_resm TBROK '$prog:$file:$start_lineno: end-of-file while reading test'");
	    print STDERR
	...
	if ($val =~ /r/ && !defined $test{$field}) {
	    system("tst_resm TBROK '$prog:$file:$start_lineno: required field \"$field\" missing'");
	    print STDERR
	...
	if ($test{'expected-fail'} !~ /^(yes|no)$/) {
	    system("tst_resm TBROK '$prog:$test{':long-name'}: bad value for expected-fail field'");
	    print STDERR
	...
	if (substr($test{'arguments'}, -1, 1) ne $firstc) {
	    system("tst_resm TBROK '$prog:$test{':long-name'}: arguments field doesn't start and end with the same character'");
	    print STDERR
		"$prog:$test{':long-name'}: arguments field doesn't start and end with the same character\n";
	    return undef;
	}
    }
    if (defined $test{'env-setup'}) {
	...
	if (substr($test{'env-setup'}, -1, 1) ne $firstc) {
	    system("tst_resm TBROK '$prog:$test{':long-name'}: env-setup field doesn't start and end with the same character'");
	    print STDERR
		"$prog:$test{':long-name'}: env-setup field doesn't start and end with the same character\n";
	    return undef;
	}
    }
	...
	if ($val =~ /^(|-)\d+$/) {
	    if ($val < 0 || $val > 255) {
		system("tst_resm TBROK '$prog:$test{':long-name'}: expected-exit value $val not in 0..255'");
		print STDERR "$prog:$test{':long-name'}: expected-exit value $val not in 0..255\n";
		return undef;
	    }
	} elsif ($val !~ /^([\s<>+-=*%\/&|!()]|\b[wse]\b|\bSIG[A-Z0-9]+\b)+$/) {
	    system("tst_resm TBROK '$prog:$test{':long-name'}: bad expected-exit expression: $val'");
    ...
    {
	system("tst_resm TBROK '$prog:$test{':long-name'}: can't use both expected-stdout and expected-stdout-pattern'");
	print STDERR
	    "$prog:$test{':long-name'}: can't use both expected-stdout and expected-stdout-pattern\n";
	return undef;
    }
    if (defined $test{'expected-stderr'}
	&& defined $test{'expected-stderr-pattern'})
    {
	system("tst_resm TBROK '$prog:$test{':long-name'}: can't use both expected-stderr and expected-stderr-pattern'");
	print STDERR
	    "$prog:$test{':long-name'}: can't use both expected-stderr and expected-stderr-pattern\n";
	return undef;
    }
    if (defined $test{'time-limit'}) {
	if ($test{'time-limit'} !~ /^\d+$/ || $test{'time-limit'} == 0) {
	    system("tst_resm TBROK '$prog:$test{':long-name'}: bad value for time-limit field'");
	    print STDERR
	...
	system("tst_resm TBROK '$prog:$test{':long-name'}: warning: duplicate test name ${test{'name'}}'");
	print STDERR
	    "$prog:$test{':long-name'}: warning: duplicate test name ${test{'name'}}\n";
    }
...
check_file_result
{
	...
	if ($type eq 'file') {
	    ...
	    if ($rest !~ /^\s*$/) {
		system("tst_resm TBROK '$prog:$test{':long-name'}: file-result test for directory $name should not have content specified'");
		print STDERR
		    "$prog:$test{':long-name'}: file-result test for directory $name should not have content specified\n";
		return undef;
	    }
	    if (-l _ || ! -d _) {
		...
		if (!defined $content) {
		    system("tst_resm TBROK '$prog:$test{':long-name'}: file-result test for $type $name failed - could not readlink - $!'");
		    print STDERR
			"$prog:$test{':long-name'}: file-result test for $type $name failed - could not readlink - $!\n";
		    return undef;
		}
