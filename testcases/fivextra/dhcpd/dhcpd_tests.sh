...
#                             problem.
#		Feb 11 2004 (rcp) determine interface dynamically; don't
#				assume eth0.
#		Feb 17 2004 (rcp) THIS TESTCASE IS DEPRECATED. It now exits
#				as BROK with a "deprecated" message.
...
#               - alias $iface to $iface:1 with IP 10.1.1.12
#
...
init()
{
	...
	# Aliasing $iface to create private network.
	/sbin/ifconfig $iface:1 10.1.1.12 &>$LTPTMP/tst_dhcpd.err || RC=$?
	if [ $RC -ne 0 ]
	then
		tst_brk TBROK "INIT: failed aliasing $iface:1 with IP 10.1.1.12"
		return $RC
	else
		/sbin/route add -host 10.1.1.12 dev $iface:1 &>$LTPTMP/tst_dhcpd.err \
			|| RC=$?
		if [ $RC -ne 0 ]
		then
...
cleanup()
{
	...
	/sbin/ifconfig | grep "$iface:1" &>$LTPTMP/tst_dhcpd.err || RC=$?
	if [ $RC -eq 0 ]
	then
		/sbin/ifconfig $iface:1 down &>$LTPTMP/tst_dhcpd.err
	fi
...
#                 /etc/dhcpd.conf file.
#				- create dhcpd.conf file, server to listen to $iface/.../10.1.1.0
#               - start dhcpd server
...
test01()
{
	...
	hwaddr=`ifconfig $iface | grep HWaddr | awk '{print $5}'` || RC=$?
	if [ $RC -ne 0 ]
	then
	...
	Listening on LPF/$iface/$hwaddr/10.1.1.0
	Sending on   LPF/$iface/$hwaddr/10.1.1.0
...
tst_resm TBROK "This testcase is deprecated and shoud not be run. try dhcp-server instead."
exit 255
