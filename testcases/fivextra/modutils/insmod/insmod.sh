...
#
# Description:  Test basic functionality of insmod command
...
#
# test01	check that insmod is installed
#
function test01()
{
	tc_register	"check that insmod is installed"
	tc_executes insmod
	tc_pass_or_fail $? "not installed"
}

#
# test02	insmod - install loadable kernel module 
#
function test02()
{
	tc_register "insmod"

	tc_exec_or_break uname cp grep lsmod rmmod || return
	tc_exist_or_break $mod_dir/dummy.o || return 

	must_remove_mod="yes"

	cp $mod_dir/dummy.o $TCTMP/$modname.o
	insmod $TCTMP/$modname.o >$stdout 2>$stderr 
	tc_fail_if_bad $? "Unexpected insmod results" || return
