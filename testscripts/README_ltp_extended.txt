...
in the $LTPROOT/doc directory in the file document_extended_ltp.txt file.  The tests that are 
...
for your architeture. This can be achieved by two ways. In the testcase/fivextra there
...
an EXCLUDE file for your system.

Another way to configure these tests are to comment/uncomment (using # ) out tests in the .scenario and .exclude configuration files 
...
generated in the $LTPROOT/results directory.  Also addtional log files will be generated 
in the /tmp directory.  One file that has a -test extention shows the testcases run(same as .scenario file). The
other file that is generated is a   ( .out) file that shows the test execution status.